...
public class late {
  ...
  public static void main(String[] args) throws Exception {
		int late_sec = Integer.parseInt(args[0]); 
    // set up the streaming execution environment
		final StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
    env.setStreamTimeCharacteristic(TimeCharacteristic.EventTime);

    // set data source
		System.out.println("\n=== Streaming with disjoint window while allowing lateness\n");
    ...
    DataStream<String> windows = ds
          ...
          .assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor<Tuple3<String, Long, Integer>>(Time.seconds(late_sec)) {
              @Override
          ...
          //.allowedLateness(Time.seconds(late_sec))
          .apply( new MyWindowFunction() )
          ;
    windows.writeAsText("hdfs://10.254.0.136:8020/C_Q2_"+String.valueOf(late_sec)+".output").setParallelism(1);

		// execute program
		env.execute("Streaming with disjoint window");
	  System.out.println("\n=== Get here! ");
  }
 ...
 **/
  private static class DataSource extends RichSourceFunction<Tuple4<Integer, Integer, Long, String>> {
    ...
    private final String filename = "/home/ubuntu/higgs-activity_time_late_arrive.txt";
