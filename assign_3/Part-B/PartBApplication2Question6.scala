...
object PartBApplication2Question6 {
  def main(args: Array[String]): Unit = {
    ...
    val popWord = wordRDD.map(_.swap).sortByKey(false).take(1)(0)._2
    
    // filter
    val res = graph.vertices.filter {
      case (id, array) => array.asInstanceOf[Set[String]] contains popWord
    }.count()
    println("number of time intervals which have the most popular word: " + res)
  }
}
