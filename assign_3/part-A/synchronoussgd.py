...
with g.as_default():
    ...
    with tf.device("/job:worker/task:0"):
        ...
        count = tf.equal(test_y, predict_y)
        norm = tf.reduce_sum(tf.mul(sparse_params, sparse_params))

    with tf.Session("grpc://vm-21-2:2222") as sess:
        ...
        for i in range(1, 1+iterate_num):
            ...
            if i % test_point == 0:
                current_error = 0
                out = open('group21_syn_error_count.csv', 'a')
                for _ in range(test_num):
                    output2 = sess.run([test_y, predict_y, count, norm])
                    is_right = output2[2] # get count
                    if not is_right:
                        current_error += 1
