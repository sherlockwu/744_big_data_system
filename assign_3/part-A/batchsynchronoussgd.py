...
test_num = int(1e2)
# test point is how often we run a test, should be 1e5
test_point = int(2e4 / batch_size)
...
input_producers = [
    ...
    ["./data/tfrecords10", "./data/tfrecords11", "./data/tfrecords12", "./data/tfrecords13"],
    ["./data/tfrecords14", "./data/tfrecords15", "./data/tfrecords16", "./data/tfrecords17"],
    ["./data/tfrecords18", "./data/tfrecords19", "./data/tfrecords20", "./data/tfrecords21"],
    ["./data/tfrecords22"]
]
...
with g.as_default():
    ...
    weight_list = [0 for i in range(5)]
    with tf.device("/job:worker/task:0"):
        for i in range(worker_num):
            index = index_list[i]
    ...
    with tf.Session("grpc://vm-21-3:2222") as sess:
        ...
        for i in range(1, 1+iterate_num):
            ...
            output = sess.run([arg1, arg2, arg3])
            if i % test_point == 0:
                current_error = 0
                out = open('group21_batched_syn_error_count.csv', 'a')
                for cur_index in range(test_num):
                    print "running iter: " + str(cur_index)
