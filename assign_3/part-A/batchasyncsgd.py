...
with g.as_default():
    ...
    with tf.Session("grpc://vm-21-%d:2222" % (FLAGS.task_index+1)) as sess:
        ...
        for i in range(1, 1+iterate_num):
            ...
            if i % test_point == 0:
                print 'reach test point'
                sess.run(update_params)
                current_error = 0
                test_point_params = w.eval()
                out = open('group21_batched_asyn_error_count.csv', 'a')
